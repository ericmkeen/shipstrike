...
# Filter to species
MR <- df %>% dplyr::filter(species == 'Humpback')
MR
...
bp <- posterior_predict(fit1,
                        ...
                        offset=rep(100, times=length(newx)))
nrow(bp) ; ncol(bp)
...
plot(pm$doy,rep(0,times=length(pm$doy)),pch=16,ylim=c(0,6.2),col="white",cex=.1,
     xlab="Day of year",ylab="Scaled encounter rate",main="_____ whales 2017 - 2020")
...
MR
save(MR,file='tests/hw/seasonal_posterior.RData')
