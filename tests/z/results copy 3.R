...
mr %>%
  ...
  summarize(penc_mean = mean(penc[is.finite(penc)], na.rm=TRUE),
            psur_mean = mean(psur[is.finite(psur)], na.rm=TRUE),
            pcol_mean = mean(pcol[is.finite(pcol)], na.rm=TRUE),
            pmor_mean = mean(pmor[is.finite(pmor)], na.rm=TRUE))
