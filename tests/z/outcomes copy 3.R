...
traffic %>% group_by(channel, type) %>% summarize(kms = sum(km))
...
# Encounter rate
lfdir <- 'tests/fw/impacts_lng_canada/proposed_speed/p_encounter/'
  ...
  for(vi in 1:length(vessels)){
    ...
    for(mi in 1:length(months)){
      ...
      for(di in 1:length(diels)){
        ...
        (fn <- paste0('tests/fw/impacts_lng_canada/proposed_speed/outcomes/',channi,'_',vessi,'.RData'))
        saveRDS(MRS, file=fn)
