...
predict_outcomes <- function(traffic,
  ...
  (penc_files <- paste0(p_encounter_dir, pencs))
    ...
    for(vi in 1:length(vessels)){
      ...
      for(mi in 1:length(months)){
        ...
        for(di in 1:length(diels)){
          ...
          if(nrow(p_encounter) > 0 & unique(p_encounter$month)[1] != 'all'){
            p_encounter <-
              p_encounter %>% dplyr::filter(month == monthi)
          }
          ...
          (fn <- paste0(outcome_dir,channi,'_',vessi,'.RData'))
          saveRDS(MRS, file=fn)
